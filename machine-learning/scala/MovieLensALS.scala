...
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    println("Starting to run MovieLensALS")
    ...
    val myRatings = loadRatings(sc, args(1))
    println("Finished loading ratings")
    ...
    val ratingsPath = movieLensHomeDir + "ratings.dat"
    println("Reading ratings from " + ratingsPath + "...")
    val ratings = sc.textFile(ratingsPath).map { line =>
      val fields = line.split("::")
      // format: (timestamp % 10, Rating(userId, movieId, rating))
      (fields(3).toLong % 10, Rating(fields(0).toInt, fields(1).toInt, fields(2).toDouble))
    }
    println("Finished reading ratings")

    val moviesPath = movieLensHomeDir + "movies.dat"
    println("Reading movies from " + moviesPath + "...")
    val movies = sc.textFile(moviesPath).map { line =>
      val fields = line.split("::")
      // format: (movieId, movieName)
      (fields(0).toInt, fields(1))
    }.collect().toMap
    println("Finished reading movies")
